		...
		<script type="text/javascript">
			$(function(){
				$("#search").click(function(){
					...
					$.post("/basepint/park/out_vehicle_condition.asp?str="+encodeURI(car_no),null,function(data){
							$("#show_data").html(data);
	 	...
	 	<div id="show_data" style="margin: 30px auto;">
	 		<table id="show_table" style="font-size: 15px; margin: 0 auto; " cellpadding="0" cellspacing="0">
		 		...
		 		<%
		 			...
		 			var sql = "SELECT b.NAME,a.PHYSICAL_NO,date_format(a.IN_TIME,'%Y-%m-%d %H:%i:%s'),a.CARD_TYPE,a.ESTABLISH_NAME,a.SERVICESET_NAME,a.REMARK FROM jsds.t_park_record_out a left join jsds.t_base_person b on a.PHYSICAL_NO = b.CARD_CODE order by IN_TIME desc limit 50;";
					var conn ,conn_str,result_set;
					/*
					 * var sql = "SELECT PHYSICAL_NO,date_format(IN_TIME,'%Y-%m-%d %H:%i:%s'),CARD_TYPE,ESTABLISH_NAME,SERVICESET_NAME,REMARK FROM jsds.t_park_record_out order by IN_TIME desc limit 100;";
					 */
